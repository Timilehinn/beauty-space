...
import RadioInputWifOtherOpt from '../../components/Inputs/radioInputWifOtherOpt'
...
import BookingReviewsComp from './BookingReviewsComp'
...
const reportOptions = [
  ...
  { value: 'Fradualent content', label: 'Fradualent content' },
  { value: 'Span or misleading', label: 'Spam or misleading' },
  { value: 'Misinformation', label: 'Misinformation' },
  { value: 'Sexual content', label: 'Sexual content' },
]

export default function BookingDetailsComponent({ id }) {
  ...
  /**
   * The function `getAllDetails` fetches workspace details, processes the data, and updates state
   * variables accordingly.
   * @returns The `getAllDetails` function is an asynchronous function that fetches workspace details
   * from an API endpoint using Axios. It processes the response data and sets various states based on
   * the received data. At the end of the function, it sets the page success, failure, and loading
   * states accordingly.
   */
  ...
  const handleOpenApp = () => {
    window.location.href = `beautyspaceng://beautyspaceng.com/booking/${workspaceData?.slug}?sid=${workspaceData?.id}`
  }
  ...
  return (
    <>
      <LoaderWithoutAuth isLoading={pageLoading} failure={pageFailure} />
      <main className='flex flex-col justify-start items-start gap-5 3xl:w-[65%] 3xl:px-10 3xl:mx-auto xxl:px-[10rem] xl:px-[10rem] lg:px-16 md:px-5 sm:px-5 pt-[7rem] relative'>
        <Breadcrumb />

        <div className='flex flex-col justify-start items-start gap-1 w-full'>
          <h1 className='text-[#011936] lg:text-4xl font-bold md:font-bold md:text-5xl md:mb-5 sm:text-2xl'>{`${workspaceData?.name}`}</h1>
      ...
      >
        <Sheet.Container>
          ...
          <Sheet.Content>
            <Sheet.Scroller>
              <div className='flex flex-col p-[20px] h-[300px] items-center gap-4'>
                <h2>Continue on BeautySpace mobile app</h2>
                <button
                  // onClick={() =>
                  //   window.open(
                  //     `https://beautyspaceng.com/booking/${workspaceData?.slug}?sid=${workspaceData?.id}`,
                  //     '_blank'
                  //   )
                  // }
                  onClick={() => handleOpenApp()}
                  className='h-14 w-full bg-primary rounded-md text-white px-5'
                >
                  Open in mobile app
                </button>
                <Link
                  target='_blank'
                  href='https://play.google.com/store/apps/details?id=com.tryspacely.tryspacely'
