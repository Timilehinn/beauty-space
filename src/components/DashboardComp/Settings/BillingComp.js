...
export default function BillingComp() {
  ...
  const calculatePriceDiff = () => {
    try {
      ...
      if (currentPlan.subscription_plan.interval === 'monthly') {
        ...
        if (planAmount > 0) {
          const dueDate = moment(currentPlan?.subscription_plan.created_at).add(
            30,
            'days'
          )
  ...
  const onSuccess = (ref, selected) => {
    upgradePlan(ref.reference, selected)
  }

  const onClose = () => {}

  const upgradePlan = async (ref, selected) => {
    try {
      setLoading(true)
      ...
      const res = await UPGRADE_SUBSCRIPTION(
        ...
        selected.identifier + planDuration,
        selected.pricing.value * multiplier - priceDiff,
        'Success',
        ref
      )
  ...
  const renderAction = (item) => {
    ...
    const shouldDowngrade =
      currentPlan.meta.pricing.value * multiplier >
        parseInt(item.pricing.value) * multiplier &&
      currentPlanInterval === interval

    if (item.identifier === 'Basic') {
      return null
    }
      ...
      <div className='w-full flex flex-col items-center'>
        {isCurrentPlan ? (
          <button
            type='button'
            onClick={() => {
              handleCancelSubscription(item)
            }}
            className='w-full rounded-full px-5 h-12 text-white bg-[tomato]'
          >
            Cancel
          </button>
        ) : (
          <div className='w-full'>
            {shouldDowngrade ? (
              <button
                ...
                className='w-full rounded-full px-5 h-12 border-[1px] border-primary text-primary bg-transparent'
              >
                Downgrade
              </button>
            ) : (
              <PaystackHook
                ...
                onSuccess={(ref) => onSuccess(ref, item)}
                onClose={onClose}
    ...
    // if(currentPlan.meta.identifier === item.identifier && interval === "yearly" && currentPlanInterval === "yearly"){
    //   return (
    //     <div className="w-full flex flex-col items-center">
    //       <button
    //         type='button'
    ...
    //         className="text-base text-primary bg-transparent mt-[15px]"
    //       >
    //         Cancel Plan
    //       </button>
    //     </div>
    ...
    //           handleCancelSubscription(item);
    //         }}
    //         className='w-full rounded-full px-5 h-12 text-white bg-[tomato] mt-[15px]'
    //       >
    //         Cancel
    //       </button>
    //     </div>
    ...
    //   var text = "Upgrade"
    //   if(item.level === currentPlan.meta.level){
    //     text = `Switch to ${currentPlan.meta.identifier} ${interval}`
    //   }
    //   if(item.level < currentPlan.meta.level){
    //     text = "Downgrade"
    //   }
  ...
  return (
    <main className='relative flex flex-col justify-start items-start gap-5 xxl:w-[60%] xl:w-[60%] lg:w-full md:w-full sm:w-full '>
          ...
          <div className='flex flex-col justify-start items-start gap-5 w-full'>
            <header className='flex justify-between items-center w-full'>
              <h2 className='text-[25px] font-semibold'>
                ...
                {currentPlan.meta.identifier !== 'Basic' && (
                  <span className='font-medium'>({currentPlanInterval})</span>
                )}
              </h2>
              {loading ? (
                <CircularSpinner />
      ...
      {loading && (
        <div className='fixed w-full h-screen top-0 left-0 bg-lightblack z-20 flex lg:justify-center lg:items-center md:justify-center md:items-center sm:justify-start sm:items-start '>
          <LoadingIndicator />
        </div>
      )}
    </main>
